...
`features/subagent.feature`: Sub-Agent 子代理系统行为规格（24 scenarios：文件发现/frontmatter 解析/名称冲突/output_protocol 提取/filter_schemas 纯函数/基础委派/工具调用/轮次耗尽/LLM 失败/output_guide 注入/元数据/工具隔离白名单黑名单防递归/token 预算/超时/生命周期注册移除上限/team_prompt/Kernel 集成）
`test_subagent.py`: subagent.feature step definitions（fixture: sactx，Mock LLM + SubAgentSystem + Kernel 集成）
