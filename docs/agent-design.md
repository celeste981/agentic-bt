...
记忆（经验/观察/偏好）    →     memory.md（单文件，倒排，100KB）
技能（学过的分析方法）    →     skills/
下属（专项委派）         →     subagents
...
不只是器官清单——器官之间有**预连通的神经系统**：OHLCV 自动流入 compute，soul 变更自动刷新 system prompt，memory 超限自动压缩。这些预连通管道是内核保证的，LLM 不需要手动搬运数据。
...
- 记录记忆 → `edit("memory.md", content)` — newest-first 倒排
- 修改灵魂 → `edit("soul.md", ...)` → 触发权限检查 + 实时刷新 system prompt
...
| 数据流 | 仿生类比 | 触发条件 | 内核行为 |
|--------|---------|---------|---------|
| soul.md → 上下文 | 性格塑造思维 | boot + soul 变更 | 注入 soul.md + WORKSPACE_GUIDE 到 system prompt |
| write:soul.md → system prompt 刷新 | 人格成长后立即生效 | soul.md 被写入/编辑时 | `_assemble_system_prompt()` 实时重建 |
| OHLCV → compute | 眼睛看到 → 大脑可用 | compute 调用时 | 自动将最近 market.ohlcv 结果注入执行环境 |
| write:memory.md → 超限压缩 | 大脑整理旧记忆 | memory.md 写入/编辑后 | 检测字数 > 100KB → LLM 压缩 → 保持倒排结构 |
...
感知 ──→ 思考 ──→ 行动 ──→ 记录 ──→ 整合 ──→ 成长
 │        │        │        │        │        │
 eyes    brain    hands   memory  compress   soul
 market   LLM   notebook memory.md  LLM   soul.md
 .ohlcv          .write  倒排写入  自动压缩  用户确认
```
...
```
"你对科技股太乐观了"
  → edit("memory.md", ...) → 记录教练反馈（newest-first）
  → 后续分析科技股时参考记忆中的审慎提示
...
kernel.wire("write:memory.md",  _on_memory_write)   # 超限自动压缩
...
input → Kernel.turn(input, session)
         │
         ├─ 1. system prompt = soul.md + WORKSPACE_GUIDE（boot 时组装，soul 变更时实时刷新）
         ├─ 2. LLM call(system_prompt + session.history, 6 tools schema)
         │      │
         │      ├─ tool_call("write", path="notebook/...", content="...")
         │      │    → permission(path) → FREE → 执行
         │      │
         │      ├─ tool_call("write", path="memory.md", content="...")
         │      │    → permission(path) → FREE → 执行
         ...
         │
         ├─ 3. session.append(new_messages)
         ├─ 4. emit("turn.done") → tracer.write
         └─ 5. return reply
```
...
Memory   = 你记住的东西（内化，脑子里的经验积累）

流动方向：
  Notebook → Memory：写完研究后，关键结论内化为记忆条目
  Memory → Notebook：基于记忆和经验，产出新的研究

反例：
  "宁德时代 Q3 产能利用率分析报告" → notebook（产物）
  "宁德产能利用率下降趋势，需关注" → memory.md（内化记忆）
```

#### Memory 设计

Memory 是**单文件 `memory.md`**，agent 自组织内容：

| 特性 | 说明 |
|------|------|
| 结构 | 单文件，agent 用 markdown 自由组织 |
| 写入顺序 | newest-first 倒排——最新条目在文件顶部 |
| 容量上限 | 100KB（MEMORY_MAX_CHARS = 100_000） |
| 读取方式 | `read("memory.md")` 自然读到最近记忆；`bash grep` 检索特定主题 |
| 超限处理 | 自动 LLM 压缩（保持倒排结构，丢弃旧细节，保留最新最重要） |
| 压缩策略 | `MemoryCompressor` Protocol — 这版 LLM，未来可换 embeddings/rules |
...
3. Skill 编排 — 工作流，不提供新能力，只组织已有工具
   /skill:research     调 market.ohlcv + compute + write + recall
   /skill:review       调 recall + read + compute
```
...
| 技能 | 命令 | 工作流 |
|------|------|--------|
| 研究 | `/skill:research {topic}` | market.ohlcv → compute 指标 → 结合 memory → notebook 写分析笔记 |
| 复盘 | `/skill:review {period}` | 回顾 observations/reflections → 生成复盘报告 |
| 扫描 | `/skill:scan {criteria}` | compute 批量筛选 → notebook 写扫描结果 |
| 对比 | `/skill:compare {a} {b}` | 两个标的并排对比分析 |
...
```
soul.md                          ← 灵魂（身份/风格/信念/边界）
memory.md                        ← 记忆（单文件，倒排，100KB，超限自动压缩）
...
| memory.md | 空文件，agent 自组织（倒排结构，100KB 上限） |
| notebook 默认目录 | research/ + reports/ + scratch/ |
| 默认 MarketAdapter | TushareAdapter |
| 默认 Skills 包 | /skill:research、/skill:review、/skill:scan、/skill:compare |
...
用户："确认执行。"
→ 系统创建 Task；每次触发生成周报 + 更新 memory.md（倒排记录复盘结论）
```

### 旅程 D：显式调用 skill

```
用户："/skill:compare 比亚迪 vs 宁德时代"

/skill:compare skill 运行：
  1. 分别获取两个标的的 OHLCV
  2. compute 计算对比指标
  3. 结合 memory 中的历史研究
...
| 自举（boot） | ✅ | soul + WORKSPACE_GUIDE 注入（Memory 内容不进入） |
...
| 管道 | 状态 | 说明 |
|------|------|------|
| soul → 上下文 | ✅ | boot + soul 变更时注入 soul.md + WORKSPACE_GUIDE |
| soul 变更 → system prompt 刷新 | ✅ | wire(soul.md) → _assemble_system_prompt() |
| OHLCV → compute | ✅ | DataStore 自动注入 |
| memory.md 超限 → 自动压缩 | ✅ | wire(memory.md) → LLMCompressor |
...
| memory 压缩可追溯 | ✅ | emit("memory.compressed", {original_chars, compressed_chars}) |
| soul 刷新可追溯 | ✅ | wire(soul.md) 触发有记录 |
